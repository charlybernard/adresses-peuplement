...
## Functions for Ontorefine

def create_ontorefine_project(ontorefine_cmd, ontorefine_url, project_name, data_file):
    cmd = f"{ontorefine_cmd} create \"{data_file}\" -u \"{ontorefine_url}\" -n \"{project_name}\""
    msg = os.popen(cmd)
...
def export_rdf_data_of_ontorefine_project(ontorefine_cmd, ontorefine_url, project_id, mapping_file, export_file):
    cmd = f"{ontorefine_cmd} rdf \"{project_id}\" -u {ontorefine_url} -m \"{mapping_file}\""
    out_content = os.popen(cmd)
    write_file(out_content.read(), export_file)

def delete_ontorefine_project(ontorefine_cmd, ontorefine_url, project_id):
    cmd = f"{ontorefine_cmd} delete \"{project_id}\" -u {ontorefine_url}"
    os.system(cmd)
